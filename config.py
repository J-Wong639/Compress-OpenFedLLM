...
class FedArguments:
    ...
    is_quantized: Optional[bool] = field(default=False, metadata={"help": "activate quantization"})
    quantize_level: Optional[int] = field(default=32, metadata={"help": "quantization level"})
    is_topk: Optional[bool] = field(default=False, metadata={"help": "activate topk compression"})
    topk_compression_ratio: Optional[float] = field(default=0.1, metadata={"help": "topk compression ratio"})
    test_dataset: Optional[str] = field(default="fpb", metadata={"help": "the test dataset to use"})
    test_batch_size: Optional[int] = field(default=16, metadata={"help": "the test batch size"})
    test_model_freq: Optional[int] = field(default=50, metadata={"help": "the frequency to test the model. 50 means save every 50 rounds"})
    subspace_change_freq: Optional[int] = field(default=4, metadata={"help": "the frequency to change gradient subspace. 4 means save every 4 rounds"})
    is_change_subspace: Optional[bool] = field(default=False, metadata={"help": "activate changing gradient subspace"})
    subspace_rank: Optional[int] = field(default=128, metadata={"help": "the rank of gradient subspace"})
...
class ScriptArguments:
    ...
    local_model_name_or_path: Optional[str] = field(default=None, metadata={"help": "the local model name"})
    ...
    special_sign: Optional[str] = field(default="special_sign", metadata={"help": "the special sign"})
...
def save_config(script_args, fed_args):
    ...
    while True:
        if not os.path.exists(output_dir):
            ...
            output_dir = f"{script_args.output_dir}/{dataset_name_split}_{fed_args.fed_alg}_c{fed_args.num_clients}s{fed_args.sample_clients}_i{script_args.max_steps}_b{script_args.batch_size}a{script_args.gradient_accumulation_steps}_l{script_args.seq_length}_qlevel{fed_args.quantize_level}_topk_ratio{fed_args.topk_compression_ratio}_{fed_args.test_dataset}_{script_args.special_sign}_{now_time}"
