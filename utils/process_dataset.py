...
def get_dataset(dataset_name, local_data_dir=None):
    ...
    elif dataset_name in ["COLA"]:
        dataset = load_dataset("iambestfeed/glue-cola", split="train")
    else:
        dataset_name = local_data_dir + dataset_name if local_data_dir is not None else dataset_name
...
def process_sft_dataset(dataset_name, dataset, dataset_sample):
    if dataset_name in ["lucasmccabe-lmi/CodeAlpaca-20k", "yahma/alpaca-cleaned", "FinGPT/fingpt-sentiment-train"]:
    ...
    elif dataset_name in ["tatsu-lab/alpaca", "vicgalle/alpaca-gpt4", "gbharti/finance-alpaca", "local_alpaca-gpt4"]:
    ...
    elif dataset_name in ["meta-math/MetaMathQA"]:
        dataset = dataset.rename_column("solution", "response")
        dataset = dataset.rename_column("original_question", "instruction")
        dataset = dataset.remove_columns(['type', 'query'])
        ...
        dataset = dataset.rename_column("output", "response")
    elif dataset_name in ["SetFit/rte", "local_RTE", "SetFit/qnli"]:
